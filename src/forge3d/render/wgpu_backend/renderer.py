...
def _lookat(eye: np.ndarray, target: np.ndarray, up: np.ndarray) -> np.ndarray:
    ...
    M[0, 3] = -float(s.dot(eye)); M[1, 3] = -float(u.dot(eye)); M[2, 3] = float(f.dot(eye))
    return M
...
class WgpuRenderer(Renderer):
    ...
    def _render_wgpu(self, snapshot: SceneSnapshot) -> np.ndarray:
        ...
        for body in snapshot.bodies:
            ...
            model_data = np.concatenate([model.T.flatten(), normal_mat.T.flatten()]).astype(np.float32).tobytes()
            ...
            mat = (snapshot.materials.get(body.material_id)
                   or BUILTIN_MATERIALS.get(body.material_id, BUILTIN_MATERIALS["default"]))
            albedo = np.asarray(mat.color[:3], dtype=np.float32)
            mat_arr = np.array([albedo[0], albedo[1], albedo[2],
                                float(mat.roughness), float(mat.metallic), 0., 0., 0.], dtype=np.float32)
            mat_buf = device.create_buffer_with_data(
                data=mat_arr.tobytes(),
                usage=wgpu.BufferUsage.UNIFORM,
            )

            bg1 = device.create_bind_group(layout=res["bgl1"], entries=[
        ...
        encoder.copy_texture_to_buffer(
            ...
            {"buffer": res["readback_buf"], "offset": 0,
             "bytes_per_row": aligned_row, "rows_per_image": H},
            (W, H, 1),
        )
