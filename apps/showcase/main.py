...
class Camera:
    ...
    def update(self, dx: float, dy: float, foot_pos, eye_h: float = 1.65):
        ...
        self.pitch = float(
            np.clip(self.pitch - dy * self.SENS, -math.pi / 2 + 0.01, math.pi / 2 - 0.01)
        )
...
class ShowcaseApp:
    ...
    def _trigger_tower(self):
        body_map = {b.name: b for b in self.world.bodies if b.name and b.name.startswith("tower_")}
    ...
    def _draw_hud(self, playing: bool):
        ...
        if not playing:
            self.viewer.draw_rect(W // 2 - 240, H // 2 - 34, 480, 68, color=(0, 0, 0), alpha=0.75)
        ...
        # Crosshair
        self.viewer.draw_rect(W // 2 - 2, H // 2 - 2, 5, 5, color=(1, 1, 1), alpha=0.65)
        ...
        for i, hint in enumerate(
            ...
            self.viewer.draw_text(
                hint, x=12, y=H - 22 - i * 22, size=13, color=(0.60, 0.60, 0.60), bg_alpha=0.0
            )
