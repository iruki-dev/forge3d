...
def _tree(w, crown, trunk, px, py, crown_mat="foliage"):
    ...
    w.add_mesh(crown, position=(px, py, 0), static=True,
               material=M[crown_mat], **_NO_COLLIDE)


def _lamp(w, mesh, px, py, mat="iron_lt", emissive="e_lamp", lamp_h=6.5):
    """Lamp post: visual-only mesh + collidable emissive sphere cap."""
    w.add_mesh(mesh, position=(px, py, 0), static=True,
               material=M[mat], **_NO_COLLIDE)
...
def _bench(w, mesh, px, py, angle=0.0, mat="wood_lt"):
    ...
    w.add_mesh(mesh, position=(px, py, 0), quat=q, static=True,
               material=M[mat], **_NO_COLLIDE)


def _rock(w, mesh, px, py, pz=0.0, mat="rock"):
    w.add_mesh(mesh, position=(px, py, pz), static=True,
               material=M[mat], **_NO_COLLIDE)


def _tower(w, base_m, mid_m, roof_m, flag_m, px, py,
           section_h=6.06, n_mid=3,
           mat_base="stone_d", mat_mid="stone", mat_roof="marble_d"):
    ...
    for mesh, mat in zip(meshes, mats):
        w.add_mesh(mesh, position=(px, py, z), static=True,
                   material=M[mat], **_NO_COLLIDE)
        z += section_h
    # Roof (OBJ height normalised to same as section_h)
    w.add_mesh(roof_m, position=(px, py, z), static=True,
               material=M[mat_roof], **_NO_COLLIDE)
    z += section_h  # roof same height as one section
    if flag_m is not None:
        w.add_mesh(flag_m, position=(px, py, z), static=True,
                   material=M["e_r"], **_NO_COLLIDE)


def _castle_wall_row(w, wall_m, px, py, count: int, axis: str = "x",
                     mat="stone_d", step: float = 4.5):
    ...
    for i in range(count):
        if axis == "x":
            ...
            wx, wy = px, py + i * step
        w.add_mesh(wall_m, position=(wx, wy, 0), static=True,
                   material=M[mat], **_NO_COLLIDE)
...
def _materials_hall(world) -> list[tuple[str, tuple]]:
    ...
    for i, (label, mat) in enumerate(PBR_DEMO):
        ...
        world.add_sphere(radius=0.72, position=(px, py, 2.66),
                         static=True, material=mat, name=f"mat_{i}")
...
def _physics_stage(world, rock_lg, campfire_m, barrel_m, crate_m) -> tuple[list, tuple]:
    ...
    for k, (oy, pz_) in enumerate([(-18, 1.0), (-13, 1.9), (-8, 2.8)]):
        _b(world, 40-k*4, 4.5, 1.0, cx, cy+oy, pz=pz_,
    ...
    # Campfire on stage
    world.add_mesh(campfire_m, position=(cx, cy+7, 0.42),
                   static=True, material=M["rock_d"])
    ...
    for bx, by, bz, use_barrel in [
        (cx-5, cy-3, 0.44, True),  (cx-7, cy-3, 0.44, False),
        (cx+5, cy-3, 0.44, False), (cx+7, cy-3, 0.44, True),
        ...
        world.add_mesh(mesh, position=(bx, by, bz), static=True,
                       material=M[mat], **_NO_COLLIDE)
...
def _scatter(world, trunk_m, crown_m, rock_lg, rock_sm):
    ...
    for y in (-4.0, 22.0, 42.0):
        _tree(world, crown_m, trunk_m, -34.0, y,
              crown_mat="foliage" if y < 30 else "foliage_a")
    for y in (-4.0, 22.0):
    ...
    for px, py, mat in [(15,-80,"rock"),(-38,12,"rock_d"),
                        (38,10,"rock"),(-8,52,"rock_d")]:
...
def _paths_boundary(world):
    ...
    for px, py, sx, sy in [( 0,-104,214,2),(0,104,214,2),
                             (-104,0,2,214),(104,0,2,214)]:
...
def _castle_perimeter(world, tw_base, tw_mid, tw_roof, flag_m, wall_m):
    ...
    _tower(world, tw_base, tw_mid, tw_roof, flag_m,  -18, -31,
           section_h=tsz, n_mid=3, mat_roof="gold")
    _tower(world, tw_base, tw_mid, tw_roof, flag_m,   18, -31,
           section_h=tsz, n_mid=3, mat_roof="gold")

    # ── Plaza corner towers — visible after entering the plaza ──────────────────
    for tx, ty in [(-36, -36), (36, -36), (-36, 36), (36, 36)]:
        _tower(world, tw_base, tw_mid, tw_roof, flag_m, tx, ty,
               section_h=tsz, n_mid=2)

    # ── Far perimeter — landmark silhouettes on the horizon ────────────────────
    for tx, ty in [( 0, -92), (92, 0), (0, 92), (-92, 0)]:
        _tower(world, tw_base, tw_mid, tw_roof, flag_m, tx, ty,
               section_h=tsz, n_mid=1)

    # ── Wall rows between nearby towers ────────────────────────────────────────
    step = tsz * 0.52
    _castle_wall_row(world, wall_m, -82, -92, count=6, axis="x",
                     mat="stone_d", step=step)
    _castle_wall_row(world, wall_m, -82,  92, count=6, axis="x",
                     mat="stone_d", step=step)
    _castle_wall_row(world, wall_m, -92, -80, count=5, axis="y",
                     mat="stone_d", step=step)
    _castle_wall_row(world, wall_m,  92, -80, count=5, axis="y",
                     mat="stone_d", step=step)
...
def build_showcase(world: f3d.World) -> ShowcaseAssets:
    ...
    world.add_box(size=(220, 220, 0.5), position=(0, 0, -0.25),
                  static=True, material=M["ground"])
