...
def _convert(src: pathlib.Path, dst: pathlib.Path,
...
def _proc(src_name: str, pack_dir: pathlib.Path,
          sx: float = 1.0, sy: float = 1.0, sz: float | None = None) -> pathlib.Path:
...
def prebuild_all() -> None:
    _all = [load_column, load_tree_crown, load_tree_trunk, load_bench,
            load_bench_long, load_lamp_post, load_potted_plant,
            load_rock_large, load_rock_small, load_campfire, load_obelisk,
            load_tower_section, load_tower_mid, load_tower_roof,
            load_wall_section, load_flag, load_barrel, load_crate,
            load_street_light]
    for fn in _all:
        fn.cache_clear()
