...
class KillFeed:
    ...
    def get_active(self, game_time: float) -> list[str]:
        return [text for text, t in self._entries if game_time - t < self._display_time]
...
class HUD:
    ...
    def _draw_health(self, viewer, player, W, H):
        ...
        # Background
        viewer.draw_rect(bar_x - 2, bar_y - 2, bar_w + 4, bar_h + 4, color=(0, 0, 0), alpha=0.60)
        ...
        viewer.draw_text(
            f"HP  {hp_val}", x=bar_x + 4, y=bar_y + 1, size=15, color=HUD_WHITE, bg_alpha=0.0
        )
        ...
        if armor_frac > 0:
            arm_y = bar_y + bar_h + 4
            viewer.draw_rect(bar_x - 2, arm_y - 1, bar_w + 4, 12, color=(0, 0, 0), alpha=0.55)
            viewer.draw_rect(bar_x, arm_y, int(bar_w * armor_frac), 10, color=HUD_BLUE, alpha=0.75)
            viewer.draw_text(
                f"ARM {int(player.armor)}",
                x=bar_x + 4,
                y=arm_y,
                size=10,
                color=HUD_WHITE,
        ...
        viewer.draw_text(
            f"Kills: {kills}", x=bar_x, y=H - 105, size=16, color=HUD_WHITE, bg_alpha=0.45
        )
    ...
    def _draw_minimap(self, viewer, player, bots, zone, W, H):
        ...
        viewer.draw_rect(
            mm_x - 1, mm_y - 1, mm_size + 2, mm_size + 2, color=(0.08, 0.12, 0.08), alpha=0.90
        )
