...
from forge3d.editor.gizmo import (
    TranslateGizmo, _ray_sphere_intersect, screen_to_ray,
)
...
def test_ray_sphere_intersect_miss():
    ...
    direction = np.array([1., 0., 0.])  # y축 방향 → 구 빗나감
    center = np.array([0., 0., 0.])
...
def test_entity_pick_no_hit():
    ...
    ew = f3d.EntityWorld()
    ew.create_entity(f3d.Transform(position=np.array([10., 10., 10.])))
...
def test_screen_to_ray():
    ...
    origin, direction = screen_to_ray(
        screen_x=640, screen_y=360,  # 화면 중앙
...
def test_gizmo_y_axis_drag():
    """Y축 드래그 → Y 좌표만 변한다."""
    ew = f3d.EntityWorld()
    e = ew.create_entity(f3d.Transform(position=np.array([1., 0., 1.])))
    gizmo = TranslateGizmo()
    gizmo.select(e)
...
def test_editor_save_scene():
    ...
    world = f3d.World()
    ew = f3d.EntityWorld()
    ew.create_entity(f3d.Transform(position=np.array([1., 2., 3.])))
    editor = f3d.EditorApp(world, ew)
